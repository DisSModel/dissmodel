...
GeoDataFrame and NumPy computation paths, not algorithmic disagreement. The
`ExperimentRecord` generated by each run captured the full execution provenance —
per-phase timings, input SHA-256 checksum, and artifact paths — automatically,
with zero additional instrumentation by the modeller.
